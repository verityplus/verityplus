    ...
    <div class="container-page">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div
          ...
          :key="plan.name"
          :class="[
            'rounded-[var(--radius-2xl)] p-8 flex flex-col transition duration-300',
            plan.featured
              ? 'relative border-2 border-primary shadow-[var(--shadow-elevated)] bg-surface md:-translate-y-4'
              : 'border border-border bg-surface hover:shadow-[var(--shadow-card-hover)]'
          ]"
        >
          <div v-if="plan.featured" class="absolute top-0 right-8 -translate-y-1/2 bg-primary text-text-inverse px-4 py-1 rounded-full text-sm font-bold">
            TERPOPULER
          </div>
          ...
          <ul class="space-y-4 mb-8 flex-grow">
            <li v-for="feature in plan.features" :key="feature.text" class="flex items-center" :class="feature.included ? 'text-text-secondary' : 'text-text-muted'">
              <i :class="feature.included ? 'bi bi-check-lg text-emerald-500' : 'bi bi-x-lg text-text-muted'" class="mr-2 text-lg"></i>
              <span :class="plan.featured && feature.included ? 'font-medium' : ''">{{ feature.text }}</span>
            </li>
          </ul>

          <button :class="plan.featured ? 'btn-primary w-full py-3 shadow-lg' : 'btn-outline w-full py-3'">
            {{ plan.cta }}
          </button>
        </div>
      </div>
