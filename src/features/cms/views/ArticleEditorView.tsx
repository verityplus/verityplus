...
export default defineComponent({
  ...
  setup() {
    ...
    const form = ref<Article>({
      ...
      category: store.categories[0] as Category, 
      author: store.authors[0] as Author,
      tags: [],
    ...
    const loadData = async () => {
      if (isEdit.value) {
        const found = await store.findBySlug(route.params.slug as string)
        if (found) {
    ...
    const toolbar = [
      ...
      { name: 'Strikethrough', icon: 'bi-type-strikethrough', action: () => insertMarkdown('~~', '~~') },
    ...
    const save = () => {
      ...
      if (isEdit.value) {
        store.updateArticle(form.value)
      } else {
      ...
      <div class="cms-editor h-full flex flex-col space-y-6 animate-in fade-in slide-in-from-bottom-10 duration-500">
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div class="flex flex-col">
            ...
            <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1 italic">Verity+ Unified Content Studio</p>
          </div>
          <div class="flex items-center gap-3">
             <button
               ...
               class={[
                 ...
                 isPreview.value ? 'bg-indigo-50 border-indigo-200 text-indigo-600 ring-2 ring-indigo-100' : 'bg-white text-slate-500'
               ]}
             >
                <i class={['bi mr-2', isPreview.value ? 'bi-eye-slash' : 'bi-eye']}></i>
                {isPreview.value ? 'Editing Mode' : 'Live Preview'}
             </button>
             <BaseButton onClick={save} variant="primary" class="shadow-lg shadow-primary/20 px-8 py-3.5 uppercase font-black tracking-widest text-xs">
        ...
        <div class="flex-grow flex flex-col lg:flex-row gap-8 min-h-[600px]">
          {/* Main Editor Column */}
          <div class={['flex-grow space-y-6 transition-all duration-500', isPreview.value ? 'lg:w-[45%]' : 'lg:w-full']}>
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-8 space-y-8">
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4 border-t border-slate-100">
                <div class="space-y-2">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">URL Slug</label>
                  <div class="relative">
                    <input 
                       disabled={isPreview.value}
                ...
                <div class="space-y-2">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Internal Category</label>
                  <select 
                    disabled={isPreview.value}
                    value={form.value.category.id}
                    onChange={(e) => { 
                       const cat = store.categories.find(c => c.id === (e.target as HTMLSelectElement).value)
              ...
              <div class="pt-4 border-t border-slate-100 min-h-[500px] flex flex-col">
                <header class="flex items-center justify-between mb-4">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Long-form Content (Markdown)</label>
          ...
          {isPreview.value && (
            <div class={['flex flex-col space-y-6 transition-all duration-500 w-full lg:w-[55%] animate-in fade-in slide-in-from-right-10']}>
               <div class="bg-white rounded-2xl border border-slate-200 shadow-xl p-8 sticky top-24 max-h-[80vh] overflow-y-auto custom-scrollbar">
                  ...
                  <div class="prose prose-slate max-w-none prose-headings:font-black prose-headings:text-slate-900 prose-p:text-slate-600 prose-p:leading-relaxed">
                     <h1 class="text-4xl font-black text-slate-900 mb-6">{form.value.title || 'Untitled Article'}</h1>
          ...
          {!isPreview.value && (
            <aside class="w-full lg:w-80 shrink-0 space-y-6 animate-in fade-in slide-in-from-right-10">
               ...
               <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
                  ...
                  <div class="space-y-4">
                     <div class="aspect-video w-full rounded-xl overflow-hidden bg-slate-100 relative group">
                        ...
                        />
                        <img src={form.value.coverImage} class="w-full h-full object-cover group-hover:scale-110 transition duration-700" />
                        <div class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center">
                           <span class="px-4 py-2 bg-white text-slate-900 rounded-lg text-xs font-black uppercase tracking-widest shadow-xl pointer-events-none">Change Asset</span>
                        </div>
                     </div>
                     <p class="text-[10px] text-slate-400 leading-tight italic uppercase font-bold tracking-widest text-center">Click Image to Upload</p>
                  </div>
               </div>
               ...
               <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-6 space-y-6">
                  ...
                  <div class="space-y-4">
                     <div class="flex items-center justify-between">
                        <span class="text-xs font-bold text-slate-500 uppercase tracking-tighter">Featured Post</span>
                        <input 
                           type="checkbox" 
                           checked={form.value.isFeatured}
                           onChange={() => { form.value.isFeatured = !form.value.isFeatured }}
                           class="w-5 h-5 rounded-md border-slate-300 text-primary focus:ring-primary/20 accent-primary" 
                        />
                     </div>
                     <p class="text-[10px] text-slate-400 leading-tight">Displayed in the primary homepage carousel.</p>
                  </div>

                  <div class="pt-6 border-t border-slate-50 space-y-3">
                     <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Author / Character</label>
                     <select 
                        value={form.value.author.id}
                        onChange={(e) => { 
                           const auth = store.authors.find(a => a.id === (e.target as HTMLSelectElement).value)
               ...
               <div class="bg-indigo-600 rounded-2xl p-6 text-white text-center shadow-lg shadow-indigo-100">
                  ...
                  <h4 class="font-black text-lg mb-1 leading-none uppercase tracking-tight">Security Note</h4>
                  <p class="text-[10px] text-white/60 mb-0 leading-relaxed uppercase font-bold tracking-widest">Admin Authorization Required for Publishing</p>
               </div>
            </aside>
          )}
        </div>
