...
export default defineComponent({
  ...
  setup() {
    ...
    const deleteUser = (id: string) => {
      ...
      if (confirm('Are you sure you want to delete this administrative user? They will lose all access to the CMS.')) {
        store.deleteUser(id)
      }
    }
      ...
      <div class="space-y-6">
        <header class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          <div class="flex flex-col">
            ...
            <p class="text-slate-400 font-medium">Manage the administrative users and access roles for the Verity+ environment.</p>
          </div>
          ...
          <div class="overflow-x-auto min-h-[400px]">
                     ...
                     <tr key={user.id} class="hover:bg-slate-50/50 transition group">
                        <td class="px-6 py-5">
                           <div class="flex items-center gap-3">
                              ...
                              <div class="flex flex-col">
                                 <span class="text-slate-900 font-bold leading-tight group-hover:text-primary transition">{user.username}</span>
                                 <span class="text-[10px] text-slate-400 mt-0.5 tracking-tighter uppercase font-medium">Administrator</span>
                              </div>
                           </div>
                        </td>
                        <td class="px-6 py-5">
                        ...
                        <td class="px-6 py-5 text-right">
                           <div class="flex items-center justify-end gap-2 translate-x-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition duration-300">
                              <router-link to={`/cms/users/${user.id}/edit`} class="w-8 h-8 rounded-lg bg-indigo-50 text-indigo-600 flex items-center justify-center hover:bg-slate-900 hover:text-white transition cursor-pointer no-underline">
             ...
             {filteredUsers.value.length === 0 && (
               <div class="py-24 flex flex-col items-center justify-center text-center">
                  ...
                  <p class="text-slate-400 font-bold italic uppercase tracking-widest">No administrative identities found.</p>
               </div>
             )}
          </div>
