...
export default defineComponent({
  ...
  setup() {
    ...
    const loadData = () => {
      if (isEdit.value) {
        const found = store.findAuthorById(route.params.id as string)
        if (found) {
          form.value = { ...found }
        }
      }
    ...
    const save = () => {
      ...
      if (isEdit.value) {
        store.updateAuthor(form.value)
      } else {
        store.addAuthor(form.value)
      }
      ...
      <div class="space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-500">
        <header class="flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div class="flex flex-col">
            ...
            <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1 italic">Identity Protection Protocol Active</p>
          </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
           <div class="bg-white p-12 rounded-2xl border border-slate-200 shadow-sm space-y-8">
              <div class="flex flex-col items-center justify-center p-8 border-4 border-dashed border-slate-100 rounded-3xl group hover:border-primary/20 transition duration-300 relative">
                ...
                />
                <div class="relative">
                   <img src={form.value.avatar} class="w-40 h-40 rounded-full border-4 border-white shadow-xl group-hover:scale-105 transition object-cover" />
                ...
                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-6 group-hover:text-primary">Click to Upload New Avatar</p>
              </div>

              <div class="space-y-6">
                 <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Public Representative Name</label>
                    <input 
                 ...
                 <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Character Biography / Backstory</label>
                    <textarea 
                      value={form.value.bio}
                      onInput={(e) => { form.value.bio = (e.target as HTMLTextAreaElement).value }}
           ...
           <div class="bg-indigo-900 p-12 rounded-2xl text-white relative overflow-hidden flex flex-col justify-center min-h-[400px] shadow-2xl">
              <div class="relative z-10 space-y-6">
                 ...
                 <h3 class="text-3xl font-black leading-tight border-b border-white/20 pb-4">Character Backstory Guidelines</h3>
                 <p class="text-indigo-100/70 text-sm leading-relaxed">
                   Verity+ authors are part of a larger narrative. Ensure the biography is engaging and provides context to why this character is an authority on their topics.
                 </p>
                 <div class="space-y-2 pt-4 border-t border-white/10">
                    <div class="flex items-center gap-3">
                       <i class="bi bi-check-circle-fill text-indigo-400" />
                       <span class="text-xs font-black uppercase">Verified Badge Automatically Applied</span>
                    </div>
                    <div class="flex items-center gap-3">
                       <i class="bi bi-check-circle-fill text-indigo-400" />
