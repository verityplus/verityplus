...
import { useCMSStore, type CMSUser } from '../store/cms.store'
      ...
      <div class="space-y-8 animate-in fade-in slide-in-from-bottom-10 duration-500 max-w-4xl mx-auto">
        <header class="flex justify-between items-center bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
          <div class="flex flex-col">
            ...
            <p class="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1 italic">Security Protocol Identity Form</p>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
           <div class="bg-white p-12 rounded-2xl border border-slate-200 shadow-sm space-y-8">
              <div class="flex flex-col items-center justify-center p-8 border-4 border-dashed border-slate-100 rounded-3xl group hover:border-primary/20 transition duration-300">
                ...
                <p class="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-2">Administrator Identity Profile</p>
              </div>

              <div class="space-y-6">
                 <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Username Reference</label>
                    <input 
                      value={form.value.username}
                      onInput={(e) => { form.value.username = (e.target as HTMLInputElement).value }}
                      type="text" 
                 ...
                 <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Administrative Email</label>
                    <input 
                 ...
                 <div class="space-y-2">
                    <label class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Credential Security (Password)</label>
                    <input 
                      value={form.value.password}
                      onInput={(e) => { form.value.password = (e.target as HTMLInputElement).value }}
                      type="password" 
           ...
           <div class="bg-slate-900 p-12 rounded-2xl text-white relative overflow-hidden flex flex-col justify-center min-h-[400px] shadow-2xl">
              <div class="relative z-10 space-y-6">
                 ...
                 <h3 class="text-3xl font-black leading-tight border-b border-white/20 pb-4">Authorization Security Architecture</h3>
                 <p class="text-slate-100/70 text-sm leading-relaxed">
                   Verity+ Administrative access tiers define the capabilities of each user. Administrators have full architectural control, while Editors focus on content creation and publication.
                 </p>
                 <div class="space-y-3 pt-4">
                    <div class="flex items-center gap-3">
                       <i class="bi bi-patch-check-fill text-emerald-400" />
                       <span class="text-xs font-black uppercase tracking-widest">Multi-Auth Integration Ready</span>
                    </div>
                    <div class="flex items-center gap-3">
                       <i class="bi bi-patch-check-fill text-emerald-400" />
                       <span class="text-xs font-black uppercase tracking-widest">Activity Auditing Protocol Active</span>
                    </div>
                 </div>
              </div>
